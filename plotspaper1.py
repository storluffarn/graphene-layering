...
#lifont = {'fontname':'serif','fontstyle':'italic'}
#lnfont = {'fontname':'serif','fontstyle':'italic'}
...
rcParams['mathtext.it'] = 'DejaVu Serif:italic'
rcParams['mathtext.bf'] = 'DejaVu Serif:bold'
...
ax.legend((f21, f22, f11, f12), ('$v = 1 \mathrm{ms}^{-1}$, std params', '$v = 2 \mathrm{ms}^{-1}$, std params', '$v = 1 \mathrm{ms^{-1}}$, mod params', '$v = 2 \mathrm{ms^{-1}}$, mod params'), loc='center right')
ax.grid(linestyle='dotted') 
...
ax2.legend((f21, f22), ('$v = 1 \mathrm{ms^{-1}}$', '$v = 2 \mathrm{ms^{-1}}$'), loc='lower right')
...
ax.legend((f3, f4, f1, f2), ('$v = 1 \mathrm{ms^{-1}}$, std params', '$v = 2 \mathrm{ms^{-1}}$, std params', '$v = 1 \mathrm{ms^{-1}}$, mod params', '$v = 2 \mathrm{ms^{-1}}$, mod params'), loc='center right')
ax.set(xlabel='$x$ (nm)', ylabel='$q$ (nm)')
...
f1, = ax.plot(v1xdata[:-7000],v1qdata[:-7000])
f2, = ax.plot(v2xdata[:-3500],v2qdata[:-3500],color='purple')
ax.legend((f1, f2), ('$v = 1 \mathrm{ms^{-1}}$', '$v = 2 \mathrm{ms^{-1}}$'), loc='lower right')
ax.set(xlabel='$x$ (nm)', ylabel='$q$ (nm)')
...
# Additional things for MRS award

fig11, (ax1, ax2) = plt.subplots(nrows=2, sharex=True)

redtdata2 = tdata2[tdata2 < 5]
redv1data = v1data[0:np.size(redtdata2)]
f11, = ax1.plot(redtdata2, redv1data, color='#ff7f0e',label='w/o substrate')
f21, = ax2.plot(tdata, fdata, color='#ff7f0e',label='with substrate')
